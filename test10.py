...
GUILD_ID = discord.Object(id=1298164729180848238)
...
async def on_ready():
    try:
        await bot.tree.sync(guild=GUILD_ID)  # 將斜線指令同步到 Discord
        print(f"已登入為 {bot.user} 並同步斜線指令。")
...
# OpenAI 聊天功能
@bot.tree.command(name="聊天", description="openai聊天功能", guild=GUILD_ID)
...
# 啟動戀愛遊戲的命令
@bot.tree.command(name="問答", description="戀愛快問快答", guild=GUILD_ID)
async def 戀愛遊戲(interaction: discord.Interaction):
    """啟動戀愛小遊戲"""
    global is_openai_enabled
    is_openai_enabled = False  # 禁用 OpenAI 回應
    
    await interaction.response.send_message("歡迎來到戀愛快問快答！讓我們開始吧！")
    await interaction.response.send_message("請輸入你要回答的題數(共有118題)")
    ...
    await interaction.response.send_message(f"已選擇 {a} 題，開始遊戲！")
    ...
    for item in selected_questions:
        ...
        await interaction.response.send_message(question_message)
        ...
        try:
            ...
            try:
                ...
                if 0 <= answer_index < len(item["answers"]):
                    ...
                    await interaction.response.send_message("這不是有效的選項，請重新選擇。")
            except ValueError:
                await interaction.response.send_message("請輸入一個有效的數字選項。")
        
        except Exception as e:
            await interaction.response.send_message("抱歉，時間過長，我們不能等待了！")
            break

    await interaction.response.send_message(f"遊戲結束！你總共得了 {score}/{len(selected_questions)} 分！")
    if score == len(selected_questions):
        await interaction.response.send_message("恭喜你！你是戀愛達人！")
    else:
        await interaction.response.send_message("不錯，繼續努力，成為戀愛高手！")
    
    is_openai_enabled = True  # 重新啟用 OpenAI 回應

# 記錄情緒
@bot.tree.command(name="記錄情緒", description="記錄情緒", guild=GUILD_ID)
async def 記錄情緒(interaction: discord.Interaction, emotion: str = None):
    # 創建下拉選單並顯示
...
# 查看情緒日曆
@bot.tree.command(name="查看情緒日曆", description="查看情緒日曆", guild=GUILD_ID)
async def 查看情緒日曆(interaction: discord.Interaction):
    ...
    for record in records:
        timestamp = datetime.datetime.strptime(record[2], '%Y-%m-%d %H:%M')  # 假設是這個格式
        formatted_time = timestamp.strftime('%Y-%m-%d %H:%M')  # 格式化為：YYYY-MM-DD HH:MM
        response += f"編號：{record[0]}，情緒：{record[1]}，時間：{formatted_time}\n"

        # 如果訊息長度超過 Discord 限制，分批發送
        if len(response) > 2000:
            await interaction.response.send_message(response)
            response = ""  # 清空回應，繼續填充下一批訊息
    # 發送最後一部分的訊息
    if response:
        await interaction.response.send_message(response)

# 刪除記錄
@bot.tree.command(name="刪除記錄", description="刪除指定的記錄", guild=GUILD_ID)
@app_commands.describe(record_ids="要刪除的記錄編號，以空格分隔，例如：1 2 3")
async def 刪除記錄(interaction: discord.Interaction, record_ids: int):
    if not record_ids.strip():
        await interaction.response.send_message("請提供要刪除的記錄編號！例如：`!刪除記錄 1 2 3`")
        return
    ...
    try:
        ...
        if deleted_count == 0:
            await interaction.response.send_message("沒有找到符合條件的記錄或記錄不屬於你！")
        else:
            await interaction.response.send_message(f"已成功刪除 {deleted_count} 條情緒記錄！")

    except ValueError:
        await interaction.response.send_message("請提供有效的記錄編號，並用空格分隔，例如：1 2 3", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message("刪除記錄時發生錯誤，請稍後再試！", ephemeral=True)
        print(f"錯誤: {e}")

# 按週分析情緒
@bot.tree.command(name="每週分析", description="顯示本週的情緒記錄", guild=GUILD_ID)
async def 每週分析(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"本週情緒分析：\n{summary}")

# 自定義區間分析情緒
@bot.tree.command(name="分析區間", description="顯示區間內的情緒記錄", guild=GUILD_ID)
...
async def 分析區間(interaction: discord.Interaction, start_date: str, end_date: str):
    ...
    await interaction.response.send_message(f"情緒分析（{start_date.strftime('%Y-%m-%d')} 到 {end_date.strftime('%Y-%m-%d')}）：\n{summary}")
